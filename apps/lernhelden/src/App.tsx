...
function readRoute():Route {
  ...
  if(parts[0]==='adventure'&&parts[1] in adventureById)return{screen:(parts[2] as Screen)||'adventure',adventureId:parts[1] as AdventureId};
  return{screen:'home'};
}
...
export function App(){
  ...
  return <div className="app" style={adventure?{'--primary':adventure.theme.primary,'--accent':adventure.theme.accent,'--surface':adventure.theme.surface,'--background':adventure.theme.background} as React.CSSProperties:undefined}>
...
export function World({adventure,save,profile,onSave}:{adventure:AdventureDefinition;save:AdventureSave;profile:PlayerProfile;onSave:(save:AdventureSave)=>Promise<void>}){
  ...
  const [position,setPosition]=useState(save.world);
  const [hint,setHint]=useState('Erkunde die Welt und finde den nächsten Gegner.');
  const chapter=currentCampaignChapter(save,adventure);
  const stats=equipmentStats(save,adventure);
  const campaignPositions=adventure.world.campaign??defaultCampaignPositions;
  ...
  const interact=()=>{if(chapter&&elite&&Math.hypot(campaignPositions.elite.x-position.x,campaignPositions.elite.y-position.y)<75){if(eliteReady)startCampaignBattle('elite');else setHint(eliteMessage);return}if(chapter&&boss&&Math.hypot(campaignPositions.boss.x-position.x,campaignPositions.boss.y-position.y)<75){if(bossReady)startCampaignBattle('boss');else setHint(bossMessage);return}const encounter=adventure.world.encounters.find(item=>Math.hypot(item.x-position.x,item.y-position.y)<75);if(encounter){sessionStorage.setItem(`lernhelden:mode:${adventure.id}`,adventure.modes[0].id);go(`/adventure/${adventure.id}/battle`);return}if(Math.hypot(adventure.world.merchant.x-position.x,adventure.world.merchant.y-position.y)<75){go(`/adventure/${adventure.id}/shop`);return}if(Math.hypot(adventure.world.chest.x-position.x,adventure.world.chest.y-position.y)<75){go(`/adventure/${adventure.id}/inventory`);return}setHint('Gehe näher an einen Gegner, Händler oder die Truhe.')};
  ...
  useEffect(()=>{const canvas=canvasRef.current;if(!canvas)return;const context=canvas.getContext('2d');if(!context)return;let active=true;context.imageSmoothingEnabled=false;const paintBackground=()=>{context.fillStyle=adventure.theme.background;context.fillRect(0,0,canvas.width,canvas.height);for(let x=0;x<canvas.width;x+=32)for(let y=0;y<canvas.height;y+=32){context.fillStyle=(x/32+y/32)%2?`${adventure.theme.primary}25`:`${adventure.theme.secondary}18`;context.fillRect(x,y,32,32)}context.fillStyle=adventure.theme.accent;context.fillRect(0,390,canvas.width,52);adventure.world.obstacles.forEach(obstacle=>{context.fillStyle=adventure.theme.surface;context.fillRect(obstacle.x,obstacle.y,obstacle.width,obstacle.height);context.fillStyle=adventure.theme.primary;context.fillRect(obstacle.x+6,obstacle.y+6,obstacle.width-12,12)})};const load=(source:string)=>new Promise<HTMLImageElement>((resolve,reject)=>{const image=new Image();image.onload=()=>resolve(image);image.onerror=reject;image.src=source});const paint=async()=>{paintBackground();const visible=adventure.world.encounters.map(entry=>({entry,enemy:adventure.enemies.find(candidate=>candidate.id===entry.enemyId)})).filter(value=>value.enemy&&!save.clearedEnemyIds.includes(value.enemy.id));const campaignFigures=[elite&&{enemy:elite,position:campaignPositions.elite,ready:eliteReady,label:'Elite'},boss&&{enemy:boss,position:campaignPositions.boss,ready:bossReady,label:'Boss'}].filter((value):value is {enemy:NonNullable<typeof elite>;position:{x:number;y:number};ready:boolean;label:string}=>Boolean(value));const avatar=avatarSprite(Math.max(0,Number(profile.avatarPresetId.split('-')[1]||1)-1));const refs=[...visible.map(value=>value.enemy!.sprite),...campaignFigures.map(value=>value.enemy.sprite),uiSprites.shop,uiSprites.chest,avatar];try{const images=await Promise.all(refs.map(ref=>load(spriteUrl(ref))));if(!active)return;const draw=(image:HTMLImageElement,x:number,y:number,size:number)=>context.drawImage(image,x-size/2,y-size/2,size,size);visible.forEach((value,index)=>draw(images[index],value.entry.x,value.entry.y,70));const campaignOffset=visible.length;campaignFigures.forEach((figure,index)=>{context.save();context.globalAlpha=figure.ready?1:.42;draw(images[campaignOffset+index],figure.position.x,figure.position.y,86);context.restore();context.fillStyle=figure.ready?'#8df0ad':'#ffe16b';context.font='bold 15px sans-serif';context.textAlign='center';context.fillText(figure.ready?figure.label:`🔒 ${figure.label}`,figure.position.x,figure.position.y-52)});const offset=campaignOffset+campaignFigures.length;draw(images[offset],adventure.world.merchant.x,adventure.world.merchant.y,58);draw(images[offset+1],adventure.world.chest.x,adventure.world.chest.y,58);draw(images[offset+2],position.x,position.y,68)}catch{if(active)paintBackground()}};void paint();return()=>{active=false}},[adventure,boss,bossReady,campaignPositions,elite,eliteReady,position,profile.avatarPresetId,save.clearedEnemyIds]);
  return <div className="page"><button className="back" onClick={()=>go(`/adventure/${adventure.id}`)}>← Zurück</button><div className="heading-row"><div><p className="eyebrow">Begehbare Pixelwelt</p><h1>{adventure.world.id.replaceAll('-',' ')}</h1></div><p>{hint}</p></div>{chapter&&<section className="campaign-map-goal"><div><small>Kampagnenziel · Kapitel {chapter.index}</small><h2>{chapter.topic}</h2><p>Missionen: {completedMissions}/{chapter.missions.length} · {eliteMessage}</p></div><div><strong>Kapitelboss</strong><p>{bossMessage}</p><p>{bossRequirement}</p></div></section>}<div className="world-wrap"><canvas ref={canvasRef} width={960} height={540}/><div className="touch-controls"><button onClick={()=>move(0,-1)}>▲</button><span><button onClick={()=>move(-1,0)}>◀</button><button onClick={interact}>E</button><button onClick={()=>move(1,0)}>▶</button></span><button onClick={()=>move(0,1)}>▼</button></div></div></div>;
}
