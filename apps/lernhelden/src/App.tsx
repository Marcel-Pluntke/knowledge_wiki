...
function MobileNavigation({route,adventure,onLogout}:{route:Route;adventure?:AdventureDefinition;onLogout:()=>void}){
  ...
  return <>
    <nav className="mobile-nav" aria-label="Hauptnavigation">
      {adventure?<>{item('Abenteuer','home',`/adventure/${adventure.id}`,active('adventure'))}{item('Karte','map',`/adventure/${adventure.id}/world`,active('world'))}{item('Beutel','bag',`/adventure/${adventure.id}/inventory`,active('inventory'))}</>:<>{item('Start','home','/home',active('home'))}{item('Erfolge','trophy','/achievements',active('achievements'))}{item('Held','user','/profile',active('profile'))}</>}
...
export function Battle({adventure,initialSave,profile,onSave,onProfile}:{adventure:AdventureDefinition;initialSave:AdventureSave;profile:PlayerProfile;onSave:(save:AdventureSave)=>Promise<void>;onProfile:(profile:PlayerProfile)=>Promise<void>}){
  ...
  if(introPanel!==null)return <div className="page battle-page"><BackButton to={`/adventure/${adventure.id}`} label="Kampf verlassen"/><BattleIntro adventure={adventure} enemy={enemy} companion={companion} panel={introPanel} onPanel={()=>setIntroPanel(1)} onBegin={()=>setIntroPanel(null)}/></div>;
  return <div className={`page battle-page battle-phase-${highestPhase+1}`}>
    <BackButton to={`/adventure/${adventure.id}`} label="Kampf verlassen"/>
    <div className="battle-heading"><div><p className="eyebrow">{adventure.modes.find(mode=>mode.id===modeId)?.title}</p><h1>{enemy.name}</h1></div><span>{enemy.place}</span></div>
  ...
  useEffect(()=>{
    ...
    const paint=async()=>{
      ...
      try{
        ...
        const offset=campaignOffset+campaignFigures.length;draw(images[offset],adventure.world.merchant.x,adventure.world.merchant.y,54);draw(images[offset+1],adventure.world.chest.x,adventure.world.chest.y,54);draw(images[offset+2],position.x,position.y,64);
