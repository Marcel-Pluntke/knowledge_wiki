...
const difficultyKey = (mode: FootballArithmeticMode) => `lernhelden:football:difficulty:${mode}`;
...
function readDifficulty(mode: FootballArithmeticMode): FootballDifficulty {
  return localStorage.getItem(difficultyKey(mode)) === 'hard' ? 'hard' : 'easy';
}
...
export function FootballGame() {
  ...
  const [difficulty, setDifficulty] = useState<Record<FootballArithmeticMode, FootballDifficulty>>(() => ({
    add: readDifficulty('add'),
    sub: readDifficulty('sub'),
  }));
  const [playback, setPlayback] = useState(false);
  const [showGoalScene, setShowGoalScene] = useState(false);
  ...
  const makeQuestion = (nextSequence: number, selectedMode = modeId) => {
    const context = {modeId: selectedMode, sequence: nextSequence, random: Math.random, chapter: 4};
    let next = fractionsAdventure.questionProvider.next(context);
    if (selectedMode !== 'add' && selectedMode !== 'sub') return next;
    const wantedCategory = difficulty[selectedMode] === 'hard' ? 'Ungleichnamige' : 'Gleichnamige';
    for (let attempt = 0; attempt < 60 && !(next.category ?? '').startsWith(wantedCategory); attempt += 1) {
      next = fractionsAdventure.questionProvider.next(context);
    }
    return next;
  };

  const changeDifficulty = (selectedMode: FootballArithmeticMode, value: FootballDifficulty) => {
    setDifficulty(current => ({...current, [selectedMode]: value}));
    localStorage.setItem(difficultyKey(selectedMode), value);
  };
  ...
  useEffect(() => {
    if (!turnLocked || !feedback) return;
    const reducedMotion = document.documentElement.classList.contains('reduced-motion')
      || (window.matchMedia?.('(prefers-reduced-motion: reduce)').matches ?? false);
    const playbackMs = reducedMotion ? 300 : lastPlayType === 'shot' ? 2450 : 1500;
    const advanceDelay = playbackMs + (reducedMotion ? 40 : 180);
    setPlayback(true);
    setAutoAdvancing(true);
    setShowGoalScene(lastOutcome === 'goal');
    document.querySelector<HTMLElement>('.football-pitch')?.scrollIntoView?.({behavior: reducedMotion ? 'auto' : 'smooth', block: 'center'});
    const timer = window.setTimeout(() => {
      setPlayback(false);
      setShowGoalScene(false);
  ...
  return <div className="football-shell" role="dialog" aria-label="Mathe Fußball">
    ...
    {!clubPanel && phase === 'select' && <main className="football-start">
      ...
      <section className="football-mode-panel">
        ...
        <section className="football-difficulty-panel" aria-label="Schwierigkeit für Addition und Subtraktion">
          <div className="football-difficulty-intro"><small>Schwierigkeit</small><strong>Nenner wählen</strong><span>Leicht = gleich · Schwer = unterschiedlich</span></div>
          <div className="football-difficulty-rows">
            {(['add', 'sub'] as const).map(arithmeticMode => <div className="football-difficulty-row" key={arithmeticMode}>
              <strong>{arithmeticMode === 'add' ? 'Addition' : 'Subtraktion'}</strong>
              <div className="football-difficulty-choices">
                {(['easy', 'hard'] as const).map(value => <button
                  type="button"
                  className={difficulty[arithmeticMode] === value ? 'selected' : ''}
                  aria-pressed={difficulty[arithmeticMode] === value}
                  onClick={() => changeDifficulty(arithmeticMode, value)}
                  key={value}
                >{value === 'easy' ? 'Leicht' : 'Schwer'}</button>)}
              </div>
            </div>)}
          </div>
        </section>
        <div className="football-mode-grid">
          {footballModes.map(mode => <button key={mode.id} onClick={() => startMatch(mode.id)}>
            <strong>{mode.title}</strong><span>{mode.id === 'add' || mode.id === 'sub'
              ? `${mode.description} · ${difficulty[mode.id] === 'hard' ? 'Schwer · unterschiedliche Nenner' : 'Leicht · gleiche Nenner'}`
    ...
    {!clubPanel && phase === 'playing' && question && <main className="football-match">
      ...
      <section className={`football-pitch football-pitch-3v3 play-${displayPlayType}${outcomeClass}${actionClass} situation-${situation}${playback ? ' football-playback' : ''}`} aria-label="3 gegen 3 Spielfeld mit aktueller Ballposition">
        <div className="football-goal left" aria-hidden="true"/>
        <div className="football-goal right" aria-hidden="true"/>
        {showGoalScene && <div className="football-goal-scene" aria-hidden="true"><strong>TOOOR!</strong><span>{playerGoals} : {opponentGoals}</span></div>}
      ...
      <section className="football-question-card">
        ...
        <form className="football-answer" onSubmit={handleSubmit}>
          ...
          <button className={`football-primary${autoAdvancing ? ' football-auto-advance' : ''}`} disabled={autoAdvancing || (!turnLocked && ((currentPlayType !== 'shot' && !selectedAction) || !playerDirection || !numerator.trim() || !denominator.trim()))} type="submit">
            {autoAdvancing ? answered >= FOOTBALL_MATCH_QUESTIONS ? 'Ergebnis folgt …' : 'Nächste Aufgabe …' : turnLocked ? answered >= FOOTBALL_MATCH_QUESTIONS ? 'Ergebnis anzeigen' : 'Nächste Aktion' : currentPlayType === 'shot' ? 'Abschluss ausführen' : 'Spielzug ausführen'}
          </button>
        </form>
