...
const provider: QuestionProvider={
  next({modeId,sequence,random,chapter = 1}){
    ...
    let an=rand(1,max),ad=rand(2,Math.min(18, 8 + chapter)),bn=rand(1,max),bd=rand(2,Math.min(18, 8 + chapter)),rn=0,rd=1,prompt='';
