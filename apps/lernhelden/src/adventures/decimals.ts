...
const provider: QuestionProvider = {
  next({modeId, sequence, random, chapter = 1}) {
    ...
    if (modeId === 'place') {
      const value = integer(9 + scale * 12) + integer(9 + scale * 12) / 100;
...
const campaignContent = createCampaign('decimals', ['Zahlen mit Komma','Zehntel','Hundertstel','Stellenwert','Vergleichen','Runden','Addieren','Subtrahieren','Geld und Maße','Zehnerpotenzen','Sachaufgaben','Komma-Festung'], ['place','compare','add','sub','shift'], ['comma-goblin','decimal-slime','hundredths-hydra','rounding-giant','zero-necromancer','decimal-dragon'], slots) as {campaign: NonNullable<AdventureDefinition['campaign']>; enemies: never[]};
