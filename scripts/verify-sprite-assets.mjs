...
if(files.length!==36)throw new Error(`Erwartet werden 36 isolierte Sprites, gefunden: ${files.length}`);
...
for(const file of files){
  ...
  if(width!==224||height!==224||bitDepth!==8||colorType!==6)throw new Error(`${file}: erwartet 224×224 RGBA`);
  ...
  for(let y=0;y<height;y++)for(let x=0;x<width;x++)if((x<12||y<12||x>=width-12||y>=height-12)&&decoded[(y*width+x)*4+3]!==0)throw new Error(`${file}: Pixel berührt den sicheren transparenten Rand`);
}
