...
let game = null, worldActive = false, worldFrame = null, keyHandler = null, dragState = null, dragGhost = null, touchInventorySelection = null;
...
function createPracticeBattle(){const index=rand(0,monsters.length-1),monster=monsters[index];return {kind:'practice',index,monster,monsterHp:7,monsterMaxHp:7,playerHp:1,maxHp:1,cooldowns:{},selectedAttack:'spark',turnPhase:'attack_select'};}
...
function attackPicker(){const battle=game.battle;if(!battle)return '';return `<section class="attack-picker" aria-label="Zauberangriff auswählen"><div class="attack-picker-heading"><strong>Wähle deinen Angriff</strong><small>Danach erscheint deine Matheaufgabe.</small></div><div class="attack-options">${battleAttacks.map(attack=>{const remaining=Math.max(0,battle.cooldowns[attack.id]||0),locked=remaining>0;return `<button type="button" class="attack-option ${locked?'locked':''}" data-attack="${attack.id}" onclick="selectBattleAttack('${attack.id}')" ${locked?'disabled':''}><strong>${attack.name}</strong><span>${attack.text}</span><small>${locked?`Noch ${remaining} ${remaining===1?'Aufgabe':'Aufgaben'} gesperrt`:'Auswählen'}</small></button>`}).join('')}</div></section>`}
function selectBattleAttack(id){if(!game?.battle||(game.battle.cooldowns[id]||0)>0)return;game.battle.selectedAttack=id;game.battle.turnPhase='question';renderGameScreen();}
...
function startGame(mode,difficulty='normal',battle=null){const resolvedBattle=battle||((practiceModes.includes(mode))?createPracticeBattle():null);if(resolvedBattle)resolvedBattle.turnPhase='attack_select';game={mode,difficulty,streak:0,number:0,battle:resolvedBattle,animating:false};nextQuestion()}
function nextQuestion(){if(game.number>0)advanceAttackCooldowns();game.number++;if(game.battle){game.battle.selectedAttack='spark';game.battle.turnPhase='attack_select';}const taskDifficulty=game.battle?.kind==='world'?(game.number<=2?'battle0':game.number<=4?'battle1':game.number<=6?'battle2':'battle3'):game.difficulty;game.q=makeQuestion(game.mode,taskDifficulty);game.taskDifficulty=taskDifficulty;game.helpStep=-1;game.helpPaid=false;game.hintPaid=false;renderGameScreen()}
function renderGameScreen(){const q=game.q,m=modes[q.type],battle=game.battle,isAttackSelection=battle?.turnPhase==='attack_select',equation=q.type==='reduce'?`${fractionHtml(q.raw)} <span class="operator">=</span> ?`:`${fractionHtml(q.a)} <span class="operator">${m.op}</span> ${fractionHtml(q.b)} <span class="operator">=</span> ?`,battleInfo=battle?`<div class="battle-stage"><div class="battle-status"><div class="battle-status-hero"><span>Magier</span><div class="health hero-health"><i class="hero-health-bar" style="width:${100*battle.playerHp/battle.maxHp}%"></i></div></div><div class="battle-status-monster"><span>${battle.monster.name}</span><div class="health"><i class="monster-health-bar" style="width:${100*battle.monsterHp/battle.monsterMaxHp}%"></i></div></div></div><canvas id="battleCanvas" class="battle-canvas" width="720" height="245"></canvas></div>`:'',helpControls=`<div class="help-actions"><button class="button secondary help-button" onclick="showTutorial()">${battle?'Hilfe anzeigen (5 🪙)':'📖 Hilfe: Schritt für Schritt'}</button>${battle?'<button class="button gold help-button" onclick="showHint()">✨ Teil-Hinweis (50 🪙)</button>':''}</div><div id="helpPanel" class="help-panel"></div>`,backAction=battle?.kind==='world'?'worldMap()':'home()',eyebrow=battle?(battle.kind==='practice'?'Übungsboss · Magische Arena':'Bosskampf · '+battle.monster.place):m.title,questionArea=!battle||!isAttackSelection?`<div class="battle-question"><div class="equation">${equation}</div><div class="answer-row"><div class="fraction-input"><input id="numerator" autocomplete="off" inputmode="numeric" aria-label="Zähler"><span></span><input id="denominator" autocomplete="off" inputmode="numeric" aria-label="Nenner"></div><button id="answerButton" class="button" onclick="checkAnswer()">Zauber wirken ✨</button></div><p class="answer-help">Zähler oben, Nenner unten. Bei einer ganzen Zahl bleibt das untere Feld frei.</p>${helpControls}<div id="feedback" class="feedback"></div><div id="hint" class="hint"></div></div>`:'';show(`<div class="${battle?'battle-page':'game-page'}">${topbar()}<div class="screen-heading"><button class="button secondary round-back" onclick="${backAction}">←</button><div><div class="eyebrow">${eyebrow}</div><h1>${battle?battle.monster.name:m.title}</h1></div></div><section class="game-panel">${battleInfo}<div class="game-turn-content"><div class="game-status"><span>Aufgabe ${game.number}</span><span>🔥 Serie: ${game.streak}</span></div><p class="mission">${battle?(isAttackSelection?'Wähle deinen Zauberangriff.':battle.kind==='practice'?'Besiege den Übungsboss mit 7 Leben. Er greift nicht zurück.':'Löse die Aufgabe und wirke einen Zauber!'):m.text}</p>${isAttackSelection?attackPicker():''}${questionArea}</div></section></div>`);const n=document.querySelector('#numerator'),d=document.querySelector('#denominator');if(n&&d){n.focus();[n,d].forEach(i=>i.addEventListener('keydown',e=>{if(e.key==='Enter')checkAnswer()}));}}
...
function playBattleAnimation(kind,damage,done,label='',spell=''){game.battleAnimation={kind,damage,label,spell,start:performance.now()};const tick=now=>{drawBattleCanvas();if(now-game.battleAnimation.start<720)requestAnimationFrame(tick);else{game.battleAnimation=null;game.animating=false;drawBattleCanvas();done()}};requestAnimationFrame(tick)}
function drawSpellEffect(ctx,spell,x,y,progress){ctx.save();if(spell==='spark'){ctx.strokeStyle='#ffe66c';ctx.lineWidth=8;ctx.lineJoin='round';ctx.beginPath();ctx.moveTo(x-16,y-15);ctx.lineTo(x+1,y-15);ctx.lineTo(x-8,y+2);ctx.lineTo(x+13,y+2);ctx.lineTo(x-15,y+23);ctx.lineTo(x-6,y+7);ctx.stroke();}else if(spell==='double'){[-11,11].forEach(offset=>{ctx.strokeStyle='#72e9ff';ctx.lineWidth=6;ctx.beginPath();ctx.moveTo(x-13,y+offset-10);ctx.lineTo(x+8,y+offset-4);ctx.lineTo(x-5,y+offset+9);ctx.lineTo(x+16,y+offset+4);ctx.stroke();});}else if(spell==='chain'){for(let i=0;i<3;i++){ctx.fillStyle=['#d7a6ff','#ab7cff','#7cf4df'][i];ctx.beginPath();ctx.arc(x-i*22,y+Math.sin(progress*13+i)*10,9-i,0,Math.PI*2);ctx.fill();}}else if(spell==='star'){ctx.fillStyle='#ff9fe7';ctx.strokeStyle='#fff0a8';ctx.lineWidth=4;ctx.beginPath();for(let i=0;i<16;i++){const angle=-Math.PI/2+i*Math.PI/8,r=i%2?10:27,px=x+Math.cos(angle)*r,py=y+Math.sin(angle)*r;i?ctx.lineTo(px,py):ctx.moveTo(px,py);}ctx.closePath();ctx.fill();ctx.stroke();}else{ctx.fillStyle='#ffe66c';ctx.beginPath();ctx.arc(x,y,13,0,Math.PI*2);ctx.fill();}ctx.restore();}
...
function drawBattleCanvas(){const c=document.querySelector('#battleCanvas');if(!c||!game?.battle)return;const ctx=c.getContext('2d'),b=game.battle,a=game.battleAnimation,elapsed=a?(performance.now()-a.start)/720:0;ctx.clearRect(0,0,c.width,c.height);ctx.fillStyle='#17244a';ctx.fillRect(0,0,c.width,c.height);for(let i=0;i<18;i++){ctx.fillStyle=i%2?'#263c6b':'#1e315b';ctx.fillRect(i*48,180+(i%3)*8,48,65)}let heroX=160,monsterX=555;if(a?.kind==='hero')monsterX+=Math.sin(elapsed*24)*10;if(a?.kind==='monster')heroX+=Math.sin(elapsed*24)*10;drawHero(ctx,heroX,115,2.15);drawMonster(ctx,b.index,monsterX,100,2.55);if(a){const start=a.kind==='hero'?220:500,end=a.kind==='hero'?500:220,progress=Math.min(elapsed*1.4,1),x=start+(end-start)*progress,y=100-Math.sin(progress*Math.PI)*45;if(a.kind==='hero')drawSpellEffect(ctx,a.spell,x,y,progress);else{ctx.fillStyle='#ff7192';ctx.beginPath();ctx.arc(x,y,13,0,Math.PI*2);ctx.fill();}ctx.fillStyle='#fff';ctx.font='bold 23px Nunito';ctx.fillText(`-${a.damage}`,a.kind==='hero'?monsterX-14:heroX-14,45);if(a.label){ctx.font='bold 15px Nunito';ctx.fillStyle='#ffeb9c';ctx.fillText(a.label,a.kind==='hero'?455:88,30)}}}

function hero(){
  touchInventorySelection=null;
  ...
  show(`${topbar()}<div class="screen-heading"><button class="button secondary round-back" onclick="home()">←</button><div><div class="eyebrow">Ziehe Gegenstände aus der Truhe auf den Helden</div><h1>Magierhaus · Inventar</h1></div></div><section class="house-inventory touch-inventory"><div class="house-sparkles">✦　✧　✦</div><article class="character-room"><div class="window-light"></div><div class="character-name">Der Brüche-Magier</div><div class="character-stage">${heroCanvas()}<div class="slot-ring">${slotMarkup}</div></div><div class="attribute-row"><span>⚔️ ${s.power}</span><span>🛡️ ${s.defense}</span><span>🍀 ${s.luck}</span></div><p class="drag-tip"><span class="desktop-inventory-tip">Ziehe ein Item auf den passenden Platz. Ziehe es zurück in die Truhe, um es abzulegen.</span><span class="mobile-inventory-tip">Tippe ein Truhen-Item an und dann auf den leuchtenden passenden Platz.</span></p><div class="tap-inventory-actions" aria-live="polite"><p id="tapInventoryStatus">Wähle ein Item in der Truhe aus.</p><button id="unequipInventoryButton" class="button secondary" type="button" disabled>In die Truhe legen</button></div></article><article class="chest-room" data-chest-drop><div class="chest-lid"></div><div class="chest-title"><span>Meine Truhe</span><small>${unequipped.length} Gegenstände</small></div><div class="chest-grid">${unequipped.length?unequipped.map(name=>{const it=items.find(i=>i.name===name);return `<button class="chest-item" data-draggable-item data-item="${it.name}" data-source-slot="chest">${itemCanvas(it)}<strong>${it.name}</strong><span>⚔️${it.power} · 🛡️${it.defense} · 🍀${it.luck}</span></button>`}).join(''):'<p class="empty-chest">Die Truhe ist leer. Ausrüstung bekommst du im Kobold-Shop.</p>'}</div><button class="button gold chest-shop-button" onclick="showShop()">Zum Kobold-Shop</button></article></section>`);
  requestAnimationFrame(setupInventoryDrag);
}
function usesTouchInventory(){return window.matchMedia?.('(pointer: coarse)').matches;}
function setupInventoryDrag(){if(usesTouchInventory()){setupInventoryTap();return;}document.querySelectorAll('[data-draggable-item]').forEach(el=>el.addEventListener('pointerdown',startInventoryDrag));}
function setupInventoryTap(){document.querySelectorAll('[data-source-slot="chest"]').forEach(el=>el.addEventListener('click',selectChestInventoryItem));document.querySelectorAll('[data-drop-slot]').forEach(el=>el.addEventListener('click',selectInventorySlot));document.querySelector('#unequipInventoryButton')?.addEventListener('click',unequipSelectedInventoryItem);updateInventoryTapUi();}
function selectChestInventoryItem(event){const source=event.currentTarget,item=items.find(candidate=>candidate.name===source.dataset.item);if(!item)return;if(touchInventorySelection?.kind==='chest'&&touchInventorySelection.item.name===item.name)touchInventorySelection=null;else touchInventorySelection={kind:'chest',item};updateInventoryTapUi();}
function selectInventorySlot(event){const slot=event.currentTarget.dataset.slot,item=equippedItem(slot),selection=touchInventorySelection;if(selection?.kind==='chest'){if(selection.item.slot!==slot){toast(`${selection.item.name} gehört zu ${slotNames[selection.item.slot]}.`);return;}data.equipped[slot]=selection.item.name;save();hero();return;}if(item){touchInventorySelection={kind:'slot',slot,item};updateInventoryTapUi();return;}toast('Wähle zuerst ein Item in deiner Truhe aus.');}
function unequipSelectedInventoryItem(){const selection=touchInventorySelection;if(selection?.kind!=='slot'||data.equipped[selection.slot]!==selection.item.name)return;delete data.equipped[selection.slot];save();hero();}
function updateInventoryTapUi(){if(!usesTouchInventory())return;const selection=touchInventorySelection,status=document.querySelector('#tapInventoryStatus'),button=document.querySelector('#unequipInventoryButton');document.querySelectorAll('[data-source-slot="chest"]').forEach(el=>el.classList.toggle('tap-selected',selection?.kind==='chest'&&el.dataset.item===selection.item.name));document.querySelectorAll('[data-drop-slot]').forEach(el=>{const matching=selection?.kind==='chest'&&el.dataset.slot===selection.item.slot;el.classList.toggle('tap-target',matching);el.classList.toggle('tap-selected',selection?.kind==='slot'&&el.dataset.slot===selection.slot);});if(selection?.kind==='chest'){status.textContent=`${selection.item.name} ausgewählt. Tippe auf den leuchtenden Platz: ${slotNames[selection.item.slot]}.`;button.disabled=true;}else if(selection?.kind==='slot'){status.textContent=`${selection.item.name} ist angelegt.`;button.disabled=false;}else{status.textContent='Wähle ein Item in der Truhe aus.';button.disabled=true;}}
