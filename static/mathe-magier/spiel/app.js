...
function startGame(mode,difficulty='normal',battle=null){game={mode,difficulty,streak:0,number:0,battle,animating:false};nextQuestion()}
function nextQuestion(){game.number++;const taskDifficulty=game.battle?(game.number<=2?'battle0':game.number<=4?'battle1':game.number<=6?'battle2':'battle3'):game.difficulty;game.q=makeQuestion(game.mode,taskDifficulty);game.taskDifficulty=taskDifficulty;game.helpStep=-1;game.helpPaid=false;game.hintPaid=false;const q=game.q,m=modes[q.type],battle=game.battle,equation=q.type==='reduce'?`${fractionHtml(q.raw)} <span class="operator">=</span> ?`:`${fractionHtml(q.a)} <span class="operator">${m.op}</span> ${fractionHtml(q.b)} <span class="operator">=</span> ?`,battleInfo=battle?`<div class="battle-status"><div><span>${battle.monster.name}</span><div class="health"><i style="width:${100*battle.monsterHp/battle.monster.hp}%"></i></div></div><div><span>Magier</span><div class="health hero-health"><i style="width:${100*battle.playerHp/battle.maxHp}%"></i></div></div></div><canvas id="battleCanvas" class="battle-canvas" width="720" height="245"></canvas>`:'',helpControls=`<div class="help-actions"><button class="button secondary help-button" onclick="showTutorial()">${battle?'Hilfe anzeigen (5 🪙)':'📖 Hilfe: Schritt für Schritt'}</button>${battle?'<button class="button gold help-button" onclick="showHint()">✨ Teil-Hinweis (50 🪙)</button>':''}</div><div id="helpPanel" class="help-panel"></div>`;show(`${topbar()}<div class="screen-heading"><button class="button secondary round-back" onclick="${battle?'worldMap()':'home()'}">←</button><div><div class="eyebrow">${battle?'Bosskampf · '+battle.monster.place:m.title}</div><h1>${battle?battle.monster.name:m.title}</h1></div></div><section class="game-panel">${battleInfo}<div class="game-status"><span>Aufgabe ${game.number}</span><span>🔥 Serie: ${game.streak}</span></div><p class="mission">${battle?'Löse die Aufgabe und wirke einen Zauber!':m.text}</p><div class="equation">${equation}</div>${bar(q.type==='reduce'?q.answer:q.a)}<div class="answer-row"><div class="fraction-input"><input id="numerator" autocomplete="off" inputmode="numeric" aria-label="Zähler"><span></span><input id="denominator" autocomplete="off" inputmode="numeric" aria-label="Nenner"></div><button id="answerButton" class="button" onclick="checkAnswer()">Zauber wirken ✨</button></div><p class="answer-help">Zähler oben, Nenner unten. Bei einer ganzen Zahl bleibt das untere Feld frei.</p>${helpControls}<div id="feedback" class="feedback"></div><div id="hint" class="hint"></div></section>`);const n=document.querySelector('#numerator'),d=document.querySelector('#denominator');n.focus();[n,d].forEach(i=>i.addEventListener('keydown',e=>{if(e.key==='Enter')checkAnswer()}));}
...
function resolveBattleHit(reward,feedback,hint){const m=game.battle.monster,damage=Math.max(1,6+stats().power-(m.rule==='golem'?2:0)),effect=m.rule==='golem'?' Der Steinpanzer fängt 2 Schaden ab.':'';game.battle.monsterHp=Math.max(0,game.battle.monsterHp-damage);updateBattleBars();game.animating=true;playBattleAnimation('hero',damage,()=>{feedback.className='feedback good';feedback.textContent=`Treffer! Dein Zauber verursacht ${damage} Schaden. +${reward} Gold!${effect}`;if(game.battle.monsterHp===0)finishBattleWin(hint);else bossCounterattack(feedback,hint)})}
function resolveBattleMiss(feedback,hint){const effect=missRuleEffect(),result=monsterAttackDamage(true),damage=result.damage,m=game.battle.monster;game.battle.playerHp=Math.max(0,game.battle.playerHp-damage);updateBattleBars();game.animating=true;playBattleAnimation('monster',damage,()=>{feedback.className='feedback try';feedback.textContent=`Nicht ganz. ${m.name} trifft dich mit ${m.attackName} für ${damage} Schaden.`;hint.textContent=[effect,result.note].filter(Boolean).join(' ')||m.ruleText;if(game.battle.playerHp===0){hint.textContent+=' Mit mehr Schutz hältst du viel länger durch.';battleButton('Zur Pixelwelt →',worldMap)}else battleButton('Neue Aufgabe →',nextQuestion)},m.attackName)}
...
function beginBattle(index){const m=monsters[index],s=stats();if(s.power<m.needPower||s.defense<m.needDefense)return showBossBriefing(index);startGame('test','hard',{index,monster:m,monsterHp:m.hp,playerHp:30+s.defense*2,maxHp:30+s.defense*2,counterCount:0,slimeHealed:false,shadowCharged:false,firePhase:false})}
